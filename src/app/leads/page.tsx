...
// jsPDF/jspdf-autotable (usados dentro de ./leadsExport) só são carregados sob demanda,
// no momento em que o usuário efetivamente exporta algo — não no carregamento da página.
...
export default function LeadsPage() {
    ...
    async function loadAllData() {
      try {
        ...
        if (user) {
          ...
          if (leadsData) {
            // O limite de 1000 evita lentidão, mas corta os leads mais antigos silenciosamente
            // quando atingido — avisamos o usuário em vez de simplesmente truncar sem indicação.
            setLeadsTruncated(leadsData.length >= 1000);
  ...
  const processExport = async (password: string | null, selectedFields: string[]) => {
    if (exportType.type === 'pdf') {
      const { handleExportPDF: handleExportPDFImpl } = await import('./leadsExport');
      handleExportPDFImpl(password, selectedFields, filteredLeads, selectedWebhookId, currentClient, () => setExportOpen(false));
    } else {
      ...
      if (exportType.type === 'csv') {
        const { convertToCSV } = await import('./leadsExport');
          ...
          <div className={styles.listSection}>
            ...
            {leadsTruncated && (
              ...
              >
                ...
                <span>Mostrando apenas os 1000 leads mais recentes. Use os filtros de data para ver leads mais antigos.</span>
              </div>
            )}
            ...
            <div className={`${styles.drawer} glass`} onClick={e => e.stopPropagation()}>
              ...
              <div className={styles.drawerFooter}>
                <div className={styles.drawerActionsLeft}>
                  ...
                  <button 
                    className={styles.secondaryBtn} 
                    style={{ background: 'rgba(0, 209, 255, 0.08)', color: 'var(--primary)', border: '1px solid rgba(0, 209, 255, 0.2)' }}
                    onClick={async () => {
                      const { exportSingleLeadPDF } = await import('./leadsExport');
                      exportSingleLeadPDF(selectedLead, currentClient);
                    }}
                  >
                    <Download size={16} />
